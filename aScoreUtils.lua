...
--	self.maxExp = 0
	...
	if type(n) == 'string' then
		...
		if e then
			for i=1,e-1 do
				if n:sub(i,i) ~= '.' then
					local v = tonumber(n:sub(i,i))
					assert(v >= 0 and v < self.base)
					self[d-1-i] = math.floor(v)
				end
			end
			self = self.shiftLeft(tonumber(n:sub(e, #n)))
...
function BigNumber:carry()
	...
	while true do
		...
		carry = 0
		while self[i] >= self.base and carry < 1000 do
			--print("Is this your problem?" .. self.base)
...
function BigNumber.longIntDiv(a,b, getRepeatingDecimals)
	...
	local digitLastRemainderPairs = {}	-- keys are digit,lastRemainder as strings
	local repeatFrom, repeatTo
...
function BigNumber.decdiv(a,b)
	return (BigNumber.longIntDiv(a,b,false))
end
...
function BigNumber.__tostring(n)
	...
	if n.maxExp > 11 then 
		s = string.sub(s, 1, 1) .. "." .. string.sub(s, 2, 4) .. "e" .. tostring(n.maxExp)
		return s
	end
	local digit = 0
	local new_s = ''
	...
	if n.minExp < 0 then
		s = s .. '.'
		for i=-1,math.max(-2, n.minExp),-1 do
			s = s .. tostring(math.floor(n[i] or 0))
		end
	end
...
function BigNumber.floor(n)
	if not n then return big(0) end
	n = BigNumber(n)
	if not n.minExp then return big(0) end
	while n.minExp < 0 do
		n[n.minExp] = 0
		n.minExp = n.minExp + 1
